...
run (const char * name)
{
   ...
   * in such way that it does not have to go through the pipe.  Of course
   * users can avoid this effect if they make the tool not produce much
   * output by adding appropriate command line options.
   */
...
move (void)
{
  ...
  if (count && count < maxidx)
	...
	if (used[i])
      ...
      if (run (tmp) == expected)
	{
	  msg ("removed %d variables", maxidx - count);
	  save ();
